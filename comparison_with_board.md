...
In the sections below, we focus on several differences that matter most for interpreting the cross-sectional comparison.
...
To us, this suggests that the main difference is **not** the construction of the predicted tariff effect per se. The exposure measures are evidently similar enough to generate a similar positive cross-sectional relationship once we align the timing and inflation definitions more closely. Below, we explore two issues more carefully:
...
Instead, the main differences appear to lie in the treatment of the dependent variable: the choice of excess-inflation baseline, the start date of the inflation window, and the use of weighting. Under the Board’s preferred choices, the cross section looks more supportive of tariff pass-through into inflation. Under nearby but still reasonable alternatives, the relationship becomes notably flatter and in some cases close to zero.
